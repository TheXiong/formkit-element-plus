...
import { onSSRComplete } from '@formkit/vue'
...
function parseSchema(
    ...
    function parseNode(
        ...
        let resolve = false
        const node = sugar(_node) as ((Exclude<FormKitSchemaNode, string | FormKitSchemaFormKit>) & {slots: Record<string, FormKitSchemaNode>})
        if (isDOM(node)) {
        ...
        // 处理 slots
        const slots: Record<string, RenderChildren> = {}
        if ('slots' in node && node.slots) {
            for (const slot in node.slots) {
                const slotNode = node.slots[slot]
                if (typeof slotNode === 'string') {
                    // We are dealing with a raw string value
                    if (slotNode.startsWith('$slots.')) {
                        // this is a lone text node, turn it into a slot
                        element = element === 'text' ? 'slot' : element
                        slots[slot] = provider(compile(slotNode))
                    } else if (slotNode.startsWith('$') && slotNode.length > 1) {
                        const value = provider(compile(slotNode))
                        slots[slot] = () => String(value())
                    } else {
                        slots[slot] = () => String(slotNode)
                    }
                } else if (Array.isArray(slotNode)) {
                    // We are dealing with node sub-children
                    slots[slot] = createElements(library, slotNode)
                } else {
                    slots[slot] = createElements(library, [slotNode])
                }
            }
        ...
        if (isComponent(node)) {
            if (children || Object.keys(slots).length > 0) {
                ...
                children = (iterationData?: Record<string, unknown>) => {
                    const obj: Record<string, (slotData?: Record<string, any>, key?: object) => RenderableList> = {}

                    const allSlots = produceChildren ? {...slots, default: produceChildren} : slots
                    for (const slotName in allSlots) {
                        obj[slotName] = (slotData?: Record<string, any>,
                            key?: object) => {
                            ...
                            const c = allSlots[slotName](iterationData)
